...
<template>
  ...
  <Histogram v-else-if="pass" :size="size" caption="core vitals" :value="{ segments: [100, 0, 0] }">
    <svg
      xmlns="http://www.w3.org/2000/svg" class="text-green-500" :width="size === 'normal' ? '6rem' : '12rem'"
  ...
  <Histogram v-else :size="size" caption="core vitals" :value="{ segments: [0, 0, 100] }">
    <svg
      xmlns="http://www.w3.org/2000/svg" class="text-red-500" :width="size === 'normal' ? '6rem' : '12rem'"
      :height="size === 'normal' ? '6rem' : '12rem'" viewBox="0 0 24 24"
    >
      <path
        fill="currentColor"
  ...
  <Histogram
    v-for="metric, key in { lcp, cls, inp }" :key="key" :size="size"
    :show-p75="showP75"
  ...
  >
    <template #caption>
      ...
      >
        ...
        <div
          :id="`tooltip-${key}`" aria-role="tooltip"
        ...
        >
          <div class="p-4 border-green-500 border w-[80vw] max-w-[500px]  rounded-lg bg-[#212121]">
            ...
            <NuxtLink
              :href="descriptions[key].link" target="_blank"
              class="text-white hover:text-green-500 focus:text-green-500 underline"
            >
              Learn more &raquo;
            </NuxtLink>
          </div>
